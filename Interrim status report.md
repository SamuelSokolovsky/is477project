...
## Overview
This interim status report summarizes the progress our team has made toward the objectives outlined in our updated project plan. Our project seeks to analyze professional soccer data from two large, complementary datasets to identify performance patterns and evaluate long-term trends to answer our research questions. Over the past several weeks, our efforts have primarily focused on dataset discovery and acquisition. This includes steps such as: validation, early integration work, exploratory cleaning, reproducibility design, and the initial preparation of the data structures needed to support our next steps.

Our progress has been shaped by challenges associated with dataset licensing and inconsistent structures across dataset candidates. After editing our project plan, we've decided to work towards building a machine learning model to approach our research questions. This has led to requiring higher standard of data quality. As a result, we dedicated significantly more time than originally planned to sourcing, validating, and assembling Dataset 2. We believe this has ultimately improved the reliability and reproducibility of our project.

## Progress on Dataset Acquisition and Preparation

### Dataset 2 Progress
The first major task in this phase involved selecting a suitable replacement for our original Dataset 2 (labeled SPI Global Rankings). Early discovery work revealed that the previously proposed SPI dataset did not meet the transparency and licensing standards required under IS 477 guidelines. With assistance from Perplexity AI, we examined multiple alternative datasets, including several listed in our dataset folder such as spi_global_rankings and club-football-match-data-2000-2025. Each of these candidates was ultimately abandoned. Note that we leverage RAG techniques to ensure that all external data sources have active links and accurately reflect their content. In most cases, licensing restrictions prevented redistribution within our workflow. In others, critical data fields were missing or were embedded in incompatible structures that prevented us from efficient evaluation.

This preliminary evaluation significantly informed our understanding of the data landscape and highlighted the need for a dataset that offered match-level depth and high schema consistency. We prioritized in first checking the data's open licensing. These constraints led us to adopt the “football-datasets” opensource repository from GitHub. This repository is authored by multiple contributors, led by Anuar Ustayev (updated 2025). The repo mirrors data from football-data.co.uk under a PDDL 1.0 public-domain license. This dataset offers over twenty-five years of match-level statistics across the five major European leagues.

We invested substantial time into reproducing and understanding the source repository with steps including: running the processing scripts, regenerating metadata packages, validating schemas, and resolving OS-specific path errors. During this process, we constructed a new consolidated master dataset that includes all leagues and all seasons. The consolidation pipeline is fully documented and reproducible. 

To regenerate our final version of Dataset 2, the user must run the notebook titled “[temp] preparation and producing consolidated Dataset2 csv file,” located in `is477project-main/data/Dataset 2/football-datasets`. This notebook loads every league’s datapackage.json, reads all corresponding CSVs, adds provenance columns, and produces a unified file named `all_leagues_all_seasons.csv`.
